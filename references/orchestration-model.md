...
The orchestrator has ten layers:
...
4. **Semantic Fact Resolution** — unresolved facts are resolved against named evidence; heuristics cannot finalize facts and negative claims require bounded proof.
5. **Decision Engine** — deterministically score six dimensions and select TRIVIAL / FAST / STANDARD / DEEP.
6. **SDD Adapter** — translate the repository's native planning artifacts into a normalized change model.
7. **Execution State Manager** — normalize native/hybrid/orchestrator execution state, guard transitions, attach analysis artifacts, record blockers/assignments/gates, append history, and support resume.
8. **Execution Policy** — compose Superpowers disciplines required by the computed flow.
9. **Quality Gates** — evaluate repository-defined automated/human gates without weakening REQUIRED policy.
10. **Evidence Ledger** — tie facts, semantic impact, decisions, state transitions, implementation, review, and verification to auditable evidence.
...
3. Explicit orchestrator/project policy.
...
CBM SEMANTIC IMPACT
  ...
  └─ IMPACT EVIDENCE → MAP INTO WORK FACTS
  ↓
...
UPDATE CURSOR / PROGRESS / SNAPSHOT
        ↓
MATERIAL DISCOVERY?
  ├─ YES → REFRESH CBM → NEW FACT/IMPACT SNAPSHOT → RECLASSIFY → RECONCILE
  └─ NO
        ↓
